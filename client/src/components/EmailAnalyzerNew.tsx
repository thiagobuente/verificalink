/**
 * Email Security Analyzer - Redesigned with SOC Dashboard Style
 * Glassmorphism + Cyberpunk + Professional Threat Intelligence
 */

import React, { useState, useEffect } from 'react';
import { Mail, AlertTriangle, CheckCircle2, ChevronDown, ChevronUp, TrendingUp, Download, Shield, AlertCircle } from 'lucide-react';
...
export default function EmailAnalyzerNew() {
  ...
  const getStatusColor = (status: string) => {
    if (status === 'valid' || status === 'seguro') return '#00ff88';
    if (status === 'suspeito') return '#ffcc00';
    return '#ff4d4d';
  };

  const getStatusIcon = (status: string) => {
    if (status === 'valid' || status === 'seguro') return '🟢';
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {/* ANALYSIS CARDS GRID - NEW DESIGN */}
      {emailInput && (
        <div className="space-y-4">
          {/* STATUS CARDS GRID */}
          <div className="email-analysis-grid" style={{
            display: 'grid',
            ...
            {/* CARD 1: REMETENTE */}
            <div className="email-analysis-card" style={{
              minHeight: '90px',
              padding: '16px',
              borderRadius: '18px',
