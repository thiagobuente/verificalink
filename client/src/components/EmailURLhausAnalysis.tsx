...
import { AlertCircle, CheckCircle, XCircle, Link as LinkIcon } from 'lucide-react';

export interface EmailURLAnalysisData {
  totalURLs: number;
...
interface EmailURLhausAnalysisProps {
  data: EmailURLAnalysisData;
  isLoading: boolean;
}
...
export function EmailURLhausAnalysis({ data, isLoading }: EmailURLhausAnalysisProps) {
  if (isLoading) {
    return (
    ...
    return (
      <div className="bg-gradient-to-br from-green-900/20 to-green-800/20 border border-green-500/30 rounded-lg p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">Análise de URLs</h3>
            <p className="text-sm text-green-400">✓ Nenhuma URL encontrada no email</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              {data.totalURLs} URL{data.totalURLs !== 1 ? 's' : ''} encontrada{data.totalURLs !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
      </div>
      ...
      {data.maliciousURLs > 0 && (
        <div className="bg-red-500/20 border border-red-500/40 rounded p-3 flex gap-2">
          ...
          <p className="text-sm text-red-300">
            ⚠️ Este email contém URLs maliciosas. Não clique em links suspeitos!
          </p>
        </div>
      )}
    </div>
